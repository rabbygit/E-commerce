            ...
            <v-row>
              <v-col>
                <v-tabs background-color="white" color="deep-purple accent-4" left>
                  <v-tab @click="Clicked('all')">All</v-tab>
                  <v-tab @click="Clicked('topay')">Unpaid</v-tab>
                  <v-tab @click="Clicked('toship')">Proccessing</v-tab>
                  <v-tab @click="Clicked('recieved')">Received</v-tab>
                  <v-tab @click="Clicked('cancelled')">Cancelled</v-tab>
                </v-tabs>
              </v-col>
              <v-col>
                <v-select
                  ...
                  <tr v-for="(product , index) in products1" :key="index">
                    <td class="px-0">
                      <div class="d-flex flex-row">
                        ...
                        <div>
                          <p class="text-subtitle-2 mt-3">{{product.product_name}}</p>
                          <p>
                            Order ID:
                            <span class="text-subtitle-2">{{product.order_id}}</span>
                          </p>
                          <p>
                            Delivery Status:
                    ...
                    <td>
                      <p class="text-center">{{product.order_status}}</p>
                    </td>
                    <td>
                      <div class="text-center">
                        <v-btn
                          v-if="product.delivery_status == 'Received'"
                          text
                          color="red"
                          nuxt
                          :to="`/user/${index}/orders/${product.id}/review`"
                        >Review</v-btn>
                      </div>
                    </td>
                  </tr>
...
export default {
  ...
  methods: {
    extractProduct(ordersArray) {
      if (ordersArray.length > 0) {
        this.products1 = [];
        for (let order = 0; order < ordersArray.length; order++) {
          for (
            let product = 0;
            product < ordersArray[order].orders.length;
            product++
          ) {
            ordersArray[order].orders[product].delivery_status =
              ordersArray[order].delivery_status;
            ordersArray[order].orders[product].order_status =
              ordersArray[order].order_status;
            this.products1.push(ordersArray[order].orders[product]);
          }
        }
      }
    },
    ...
    Clicked(value) {
      ...
      switch (value) {
        case "all":
          this.$axios
            .post(`https://tango99.herokuapp.com/Cart/allorders/`, {
              user_id: 1,
            })
            .then((response) => {
              if (response.data.success) {
                this.extractProduct(response.data.data);
              } else {
                this.products1 = [];
              }
            });
        ...
        case "topay":
          this.$axios
            .post(`https://tango99.herokuapp.com/Cart/orders_to_pay/`, {
              user_id: 1,
            })
            .then((response) => {
              if (response.data.success) {
        ...
        case "toship":
          this.$axios
            .post(`https://tango99.herokuapp.com/Cart/orders_to_ship/`, {
              user_id: 1,
            })
            .then((response) => {
              if (response.data.success) {
        ...
        case "recieved":
          this.$axios
            .post(`https://tango99.herokuapp.com/Cart/orders_received/`, {
              user_id: 1,
            })
            .then((response) => {
              if (response.data.success) {
        ...
        case "cancelled":
          this.$axios
            .post(`https://tango99.herokuapp.com/Cart/orders_cancelled/`, {
              user_id: 1,
            })
            .then((response) => {
              if (response.data.success) {
  ...
  computed: {
    filtered() {
      return this.products1.slice(1).slice(-this.limit);
    },
  },
  created() {
    this.$axios
      .post(`https://tango99.herokuapp.com/Cart/allorders/`, { user_id: 1 })
      .then((response) => {
        if (response.data.success) {
          let ordersArray = response.data.data;
          if (ordersArray.length > 0) {
            for (let order = 0; order < ordersArray.length; order++) {
              for (
                let product = 0;
